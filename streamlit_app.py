...
def render_trial(trial_number: int) -> None:
    ...
    with st.form(f"trial_form_{trial_number}", border=False):
        ...
        concerns = st.multiselect(
            "Which of the following concerns or needs are present in this event and should influence the robot's decision? Select all that apply.",
            SITUATION_CONCERNS + [NO_SPECIAL_CONCERN, NOT_ENOUGH_CONCERN_INFO, "Other (please specify)"],
        ...
        for row_number, row in enumerate(attribute_rows):
            attribute_answers[row] = st.selectbox(
                row,
                ["— Select one —"] + ATTRIBUTE_SCALE,
